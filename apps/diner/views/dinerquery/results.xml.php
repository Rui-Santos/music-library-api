...
$root_element->setAttribute("term",str_replace("brktzspaceztkrb", " ", $dinerquery->searchTerm));
...
//foreach($dinerquerySet as $dinerquery):
	...
	$itemElement->setAttribute("number",$i);
	...
	$artworkURIElement->appendChild($doc->createTextNode(utf8_encode("http://208.65.156.27/THE_DINER/ART_small/" . $dinerquery->CDTitle . ".jpg")));
	...
	$relevanceElement->appendChild($doc->createTextNode(utf8_encode(floor($dinerquery->Relevance))));
	$itemElement->appendChild($relevanceElement);

	$ratingElement = $doc->createElement("rating");
	$ratingElement->appendChild($doc->createTextNode(utf8_encode(floor($dinerquery->Rating))));
