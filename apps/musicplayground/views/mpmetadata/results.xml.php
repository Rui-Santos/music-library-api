...
$root_element->setAttribute("term",$mpmetadata->searchTerm);
...
for ($i=$startNum; $i <= $limit; $i++) {
	...
	if ($a && $a->status=="active") {
    	...
    	$CategoryElement->appendChild($doc->createTextNode(utf8_encode($mpmetadata->Category)));
    	...
    	if(isset($mpmetadata->Relevance)) {
    		$relevanceElement = $doc->createElement("relevance");
    		$relevanceElement->appendChild($doc->createTextNode(utf8_encode(floor($mpmetadata->Relevance))));
