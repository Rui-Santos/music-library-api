...
for ($i=$startNum; $i <= $limit; $i++) {
	...
	$CategoryElement->appendChild($doc->createTextNode(utf8_encode($mpquery->Category)));
	...
	$relevanceElement->appendChild($doc->createTextNode(utf8_encode(floor($mpquery->Relevance))));
	$itemElement->appendChild($relevanceElement);

	if (count($mpquery->assets()) > 0) {
		$assetkeyElement = $doc->createElement("asset_key");
		$assetkeyElement->appendChild($doc->createTextNode(utf8_encode($mpquery->assets()->orderBy('id DESC')->first()->asset_key)));
