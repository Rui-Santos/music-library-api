...
class SqlBuilder implements ISqlConditions, ISqlSelectOptions {
	...
	protected $limit;
	protected $dinerField = '*';
	protected $dinerWhereAdditions = '';
	protected $dinerMatch = '';
	protected $offset;
	protected $distinct;
...
/* 		$sql = 'SELECT ' . $this->distinct . self::escapeWithTicks($this->select); */
		$sql = 'SELECT ' . $this->distinct . $this->dinerField;
		...
		$sql .= $this->dinerMatchHelper();
		
		$sql .= $this->whereHelper();
		
		$sql .= $this->dinerWhereAdditionsHelper();
	 ...
	 * LIMIT results to some number of records.
	 *
	 * @param string $dinerFields
	 * @return SqlBuilder
	 */
	public function dinerField($dinerFields)           { 
		$this->dinerField = $dinerFields;
		if(strpos($this->dinerField,'Manufacturer')=== false && $this->table =='metadata' ) {
			$this->dinerField .= ', Manufacturer';
		}
		return $this; 
	}
	 ...
	 * LIMIT results to some number of records.
	 *
	 * @param string $dinerWhere
	 * @return SqlBuilder
	 */
	public function dinerWhereAdditions($dinerWhere)           { $this->dinerWhereAdditions = $dinerWhere; return $this; }
	
	/**
	 * MATCH results to search term.
	 *
	 * @param string $dinerMatchTerm
	 * @param string $dinerMatchConditions
	 * @param string $dinerMatchLibrary
	 * @return SqlBuilder
	 */
	public function dinerMatch($dinerMatchTerm, $dinerMatchConditions, $dinerMatchLibrary)           { $this->dinerMatch = $dinerMatchTerm; $this->dinerMatchConditions = $dinerMatchConditions; $this->dinerMatchLibrary = $dinerMatchLibrary; return $this; }
	...
	protected function join($leftOrRight, $innerOrOuter, $table, $tablePrimaryKey, $fromTableForeignKey) {
		$fromTableAfterJoin = substr($tablePrimaryKey, 0, strpos($tablePrimaryKey, '.'));
	 ...
	 * @return SqlBuilder
	 */
	public function selectAs($select, $as, $match=false, $table='diner') {
		if (!$match) {
			$this->selectAs[$as] = $select . ' as ' . $as;
		} else {
			$matchHelperTerm = $select;
		
			$matchArray = explode(" ", $matchHelperTerm);
			foreach($matchArray as &$matchTerm) {
/* 				$matchTerm = str_replace(',', '', $matchTerm); */
/* 				$matchTerm = str_replace('[space]', ' ', $matchTerm); */
				$matchTerm = str_replace("'", "", $matchTerm);
				
				$thesaurusCheck = preg_grep("/ $matchTerm /i",$this->thesaurus);
				$thesaurusCheck = array_values($thesaurusCheck);
				if ($thesaurusCheck) {
					$matchTerm = "(".substr( substr($thesaurusCheck[0], 1), 0, -1).")";
				}
				$matchTerm = "+".$matchTerm;
				$matchTerm = str_replace('brktzspaceztkrb', ' ', $matchTerm);
/* 				$matchTerm = str_replace('"', '\"', $matchTerm); */
			}
			$matchHelperTerm = implode(" ", $matchArray);
/* 			$matchHelperTerm = str_replace("[apos]","'",$matchHelperTerm); */
			
			switch($table) {
				case 'diner':
			
			
					$fullMatch = 'MATCH(Category) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 800 + ';
					//$fullMatch .= 'MATCH(CDTitle) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 400 + ';
					$fullMatch .= 'MATCH(Description,FeaturedInstrument) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 400 + ';
					$fullMatch .= 'MATCH(Composer,Keywords,Library,Mood) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 100';
					$fullMatch .= ' + (Rating*2) - 600';
					break;
					
				case 'prosfx':
					$fullMatch = 'MATCH(Category) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 8 + ';
					$fullMatch .= 'MATCH(CDTitle) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 4 + ';
					$fullMatch .= 'MATCH(TrackTitle) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 2 + ';
					$fullMatch .= 'MATCH(Description,Filename,Keywords,Library,Mood) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 1';
					break;
				case 'musicplayground':
					//$fullMatch = 'MATCH(Description) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE)';

					$fullMatch = 'MATCH(Category) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 800 + ';
					$fullMatch .= 'MATCH(Library) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 400 + ';
					$fullMatch .= 'MATCH(Description) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 200 + ';
					//$fullMatch .= 'MATCH(Lyrics) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 2 + ';
					$fullMatch .= 'MATCH(Composer,Filename,Keywords,Mood,TrackTitle) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) * 100';
					$fullMatch .= ' + ((Rating - 3)*100)';
/* 					$fullMatch = 'Rating - 3'; */
			}
			$this->selectAs[$as] = $fullMatch . ' AS ' . $as;
		}
		return $this;
	}
	...
	protected function whereHelper() {
		...
		$h = getallheaders();
		$country = empty($h['COUNTRY']) ? 'US' : $h['COUNTRY'];
		if($this->table == 'metadata') {
			switch($country) {
				case 'US':
					...
					break;
				case 'CA':
					if($sql != '' || $this->dinerMatch) {
						$sql .= " AND Manufacturer NOT IN ('Mode')";
					} else {
						$sql .= "Manufacturer NOT IN ('Mode')";
					}
					break;
				default:
					if($sql != '' || $this->dinerMatch) {
						$sql .= " AND Manufacturer NOT IN ('Apollo','Mode','Skumba')";
					} else {
						$sql .= "Manufacturer NOT IN ('Apollo','Mode','Skumba')";
					}
			}
			if($d['username']=='tunetruck' || $d['username']=='takeourmusic') {
				if($sql != '' || $this->dinerMatch) {
					$sql .= " AND Conductor NOT REGEXP 'NL|NP|SL' AND SubCategory NOT LIKE '%album%'";
				} else {
					$sql .= "Conductor NOT REGEXP 'NL|NP|SL' AND SubCategory NOT LIKE '%album%'";
				}
			}
		}
...
/* 			} */
	...
	protected $thesaurus = Array(
			" anthem anthemic ",
			" orchestra orchestral ",
			' fast upbeat uptempo "upbrktzspaceztkrbtempo" "fastbrktzspaceztkrbtempo" "upbrktzspaceztkrbbeat" ',
			" electro electronic electronica ",
			" vinyl scratch scratches scratching ",
			" tympani timpani ",
			" effects fx sfx ",
			" inspiring inspirational aspirational ",
			" motivating motivational ",
			" build building ",
			" ukulele ukelele uke ",
			" jazz jazzy ",
			" soul soulful ",
			' rock rocking "rockbrktzspaceztkrbandbrktzspaceztkrbroll" "rockbrktzspaceztkrbnbrktzspaceztkrbroll" ',
			' r&b rnb "rbrktzspaceztkrbandbrktzspaceztkrbb" "rhythmbrktzspaceztkrbandbrktzspaceztkrbblues" R&B ',
			' pizz pizzicato "pizzicatobrktzspaceztkrbstrings" "pluckedbrktzspaceztkrbstrings" "pizzbrktzspaceztkrbstrings" ',
			' vox vocal vocals ',
			' male "malebrktzspaceztkrbvox" "malebrktzspaceztkrbvocals" "malebrktzspaceztkrbvocal" ',
			' female "femalebrktzspaceztkrbvox" "femalebrktzspaceztkrbvocals" "femalebrktzspaceztkrbvocal" ',
			' "steelbrktzspaceztkrbdrum" "steelbrktzspaceztkrbdrums" ',
			' drum drums drumkit drumset "drumbrktzspaceztkrbkit" "drumbrktzspaceztkrbset" drumbeat "drumbrktzspaceztkrbbeat" ',
			' "groupbrktzspaceztkrbvox" "groupbrktzspaceztkrbvocals" "malebrktzspaceztkrbfemalebrktzspaceztkrbvocals" "malebrktzspaceztkrbandbrktzspaceztkrbfemalebrktzspaceztkrbvocals" "malebrktzspaceztkrbvocalsbrktzspaceztkrbandbrktzspaceztkrbfemalebrktzspaceztkrbvocals" "malebrktzspaceztkrbandbrktzspaceztkrbfemalebrktzspaceztkrbvox" "malebrktzspaceztkrbfemalebrktzspaceztkrbvox" ',
			' "slapbrktzspaceztkrbbass" "slapbrktzspaceztkrbelectricbrktzspaceztkrbbass" ',
			' opera operatic "operabrktzspaceztkrbsinger" "operabrktzspaceztkrbvoice" "operabrktzspaceztkrbvocals" "operabrktzspaceztkrbvocal" ',
			' fart farts farting "drewbrktzspaceztkrbstein" ',
		);

	
	protected function dinerMatchHelper() {
		// multi word terms separated by brktzspaceztkrb
	
		$sql = '';
		if(!empty($this->dinerMatch)){
		
			$matchHelperTerm = $this->dinerMatch;
...
/* 			$matchHelperTerm = str_replace("[apos]","'",$matchHelperTerm); */
			...
			switch($this->dinerMatchLibrary) {
				case 'diner':
					$sql = ' WHERE MATCH(Category,Composer,Description,Filename,Keywords,Library,Mood,Pathname,SubCategory,TrackTitle,CDTitle,Manufacturer,ShortID) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) ' . $this->dinerMatchConditions;
					break;
				case 'users':
					$sql = ' WHERE MATCH(user,username,email,company,phone,country,diner_sites) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) ' . $this->dinerMatchConditions;
					break;
				case 'mpsamplers':
				case 'dinerpostings':
					$sql = ' WHERE MATCH(name,description,notes) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) ' . $this->dinerMatchConditions;
					break;
				case 'dinerpostinglogs':
					$sql = ' WHERE MATCH(posting_account,email,ip,asset) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) ' . $this->dinerMatchConditions;
					break;
				default:
					$sql = ' WHERE MATCH(Category,Composer,Description,Filename,Keywords,Library,Mood,Pathname,SubCategory,TrackTitle,CDTitle,Manufacturer,ShortID) AGAINST(\'' . $matchHelperTerm . '\' IN BOOLEAN MODE) ' . $this->dinerMatchConditions;
					break;
	...
	protected function dinerWhereAdditionsHelper() {
		// multi word terms separated by brktzspaceztkrb
		...
		if(!empty($this->dinerWhereAdditions)){
		
			$sql=$this->dinerWhereAdditions;
